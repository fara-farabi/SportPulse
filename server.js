...
app.get('/api/cricket/matches', async (req, res) => {
  try {
    ...
    const inferTeamsFromTournament = (tournament) => {
      if (!tournament) return ['Unknown', 'Unknown'];

      const cleanTournament = tournament
        .replace(/,\s*\d{4}/, '')
        .replace(/\s+\d{4}$/, '')
        .trim();

      if (cleanTournament.toLowerCase().includes(' tour of ')) {
        const parts = cleanTournament.split(/ tour of /i);

        const team1 = parts[0]?.trim() || 'Unknown';
        const team2 = parts[1]?.trim() || 'Unknown';
    ...
    let results = data.map(row => {
      const tournamentName = getValue(row, [
        'Tournament',
        'League',
        'Series'
      ]);

      let team1 = getValue(row, [
        'Team 1',
        'Team1',
        'team_1',
        'Home Team',
      ...
      const inferredTeams = inferTeamsFromTournament(tournamentName);

      if (!team1 || team1.toLowerCase() === 'unknown') {
        team1 = inferredTeams[0];
      }

      if (!team2 || team2.toLowerCase() === 'unknown') {
        team2 = inferredTeams[1];
      }

      return {
        id: getValue(row, ['Match_ID', 'Match ID', 'id']),
        tournament: tournamentName || 'Unknown Tournament',
        stadium: getValue(row, ['Stadium', 'Venue', 'Ground']) || 'Unknown Stadium',
        team1: team1 || 'Unknown',
        team2: team2 || 'Unknown',
        team1Score: getValue(row, [
          'Team 1 Score',
          'Team1 Score',
          'Home Score',
          'Score 1'
        ]) || '0/0',
        team2Score: getValue(row, [
          'Team 2 Score',
          'Team2 Score',
          'Away Score',
          'Score 2'
    ...
    if (tournament) {
      results = results.filter(r =>
        (r.tournament || '').toLowerCase().includes(tournament.toLowerCase())
      );
    }
    ...
    console.error('Cricket archive error:', err);
    res.status(500).json({
      success: false,
      message: err.message
    });
